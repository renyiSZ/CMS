...
<html>
  <head>
    ...
    <title>My JSP 'GroupShare.jsp' starting page</title>
  ...
  <% ArrayList classlist = (ArrayList)session.getAttribute("classlist");  
   List groupMemberList=(List)request.getAttribute("groupMemberList");
 ...
 <form name="formname" action="<%=path%>/StudentGroupShare" method="post" >
 ...
 <option value="<%=currentclass %>"  ><%=currentclass %></option> 
			...
			<%
			 ...
			 %>	
</select><input type="hidden" name="type" value="group">
<button onclick="javascript:document.formname.submit();">&nbsp;Search&nbsp;</button> </form>
   ...
   <tr>
    ...
    <%
		...
		HashMap p= (HashMap) groupMemberList.get(0);
		String groupname=""+p.get("groupname");
	%>
		<td> Group <%=groupname %> Members </td>
	<% 
			for(int i=0;i<groupMemberList.size();i++){
				HashMap m= (HashMap) groupMemberList.get(i);
     %>
    
    <td><%= m.get("studentname")%>&nbsp;(<%= m.get("studentid")%>)</td>
...
<%
		if(groupMemberList!=null&&groupMemberList.size()>0){
		 HashMap map=(HashMap)groupMemberList.get(0);
          String group=""+map.get("groupname");
%>
  <div style="  border: 1px solid #99D3F5; width:200px; padding:5px; ">
  <form action="<%=path %>/UploadGroupShare" enctype="multipart/form-data" name="uploadform" method="post" onsubmit="return doSubmit()">
   		<a href="javascript:;" class="file">
   		<i class="fa fa-plus " aria-hidden="true"></i> &nbsp;Your Files
...
<%
    ...
    <div class="videodiv">
	 <div class="imgdiv">
	 	<a href="<%=path%>/UploadGroupShare?downloadID=<%=m.get("shareid")%>">
	 ...
	 &nbsp;<%=m.get("uploadername")%></br>
	  ...
	  <a href="<%=path%>/ShareServlet?type=deletenote&noteid=<%=m.get("shareid")%>&kc=<%=currentclass%>&person=studentgroup">
