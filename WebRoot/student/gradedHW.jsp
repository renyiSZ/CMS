...
<html>
 ...
 %>
  ...
  <div class="admin-main">
    ...
    <button class="layui-btn" onClick="javascript:window.location.href='<%=path%>/StudentHomeworkServlet?require=graded&action=allClassHW'">
    	All Homework
    </button>&nbsp;&nbsp;
 	<%	for (int ii=0;ii<classlist.size();ii++){
 	 %>
    <button class="layui-btn" onClick="javascript:window.location.href='<%=path%>/StudentHomeworkServlet?require=graded&action=singleClassHW&classid=<%=classlist.get(ii) %>'">
    	<%=classlist.get(ii) %>
    </button>&nbsp;&nbsp;
    <%
    	} 
...
<%
 ...
 %>
  	    ...
  	    <div style="width:80%; margin:15px; padding:10px;  border:2px solid #E5E5E5; word-wrap:break-word;">
   		...
   		</br><font color="#53868B"><strong>Mark:</strong></font>&nbsp;<%=m.get("result") %>
   		&nbsp;&nbsp;&nbsp;&nbsp;<font color="#53868B"><strong>Correct time:</strong></font>&nbsp;<%=m.get("correcttime") %>
   		</br></br>
   		<font color="#53868B"><strong>Comment:</strong></font>&nbsp;<%=m.get("comment") %></br></br>
   		</div>
 <%
 }}else{
  %>
